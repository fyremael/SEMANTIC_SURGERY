...
def run(seed: int = 0, dim: int = 64, n_samples: int = 256, alpha: float = 0.3) -> ProbeResult:
    ...
    metrics = {
        "target_margin_delta": float(target_delta),
        "off_target_delta": float(off_target_delta),
        "off_target_degradation_max": float(max(0.0, -off_target_delta)),
        "causal_locality_score": float(locality_ratio),
    }
    thresholds = {
        "off_target_degradation_max": PHASE0_DEFAULT_THRESHOLDS["off_target_degradation_max"],
        "target_margin_delta_min": 0.20,
        "causal_locality_score_min": 10.0,
    }
    warnings: list[str] = []
    accepted = True
    ...
    if metrics["target_margin_delta"] < thresholds["target_margin_delta_min"]:
